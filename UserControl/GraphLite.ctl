...
For i = 0 To Columns - 1 Step m_HorizontalTickFrequency
   ...
   UserControl.Print PlotData(0, i)
Next i
...
' Purpose   :
'---------------------------------------------------------------------------------------
Public Property Get DisplayAT() As Boolean

On Local Error GoTo DisplayAT_Error
DisplayAT = m_bDisplayAT

On Error GoTo 0
Exit Property

DisplayAT_Error:
If bDebug Then Handle_Err Err, "DisplayAT-GraphLite"
Resume Next
...
' Purpose   :
'---------------------------------------------------------------------------------------
Public Property Let DisplayAT(ByVal bDisplayAT As Boolean)

On Local Error GoTo DisplayAT_Error
m_bDisplayAT = bDisplayAT
Call UserControl.PropertyChanged("DisplayAT")
...
' Purpose   :
'---------------------------------------------------------------------------------------
Private Function GetSlope(ByVal lArrayPos As Long, ParamArray UData() As Variant) As Currency
On Local Error GoTo GetSlope_Error
...
lLastItem = UBound(UData(0), 2) - 1

For lLoop = 0 To lLastItem
    ' Do access pass last item
    If lLoop < lLastItem Then
        cResult = cResult + (UData(0)(1, lLoop + 1) - UData(0)(1, lLoop)) / (UData(0)(2, lLoop + 1) - UData(0)(2, lLoop))
    End If
Next lLoop

GetSlope = cResult / lLastItem

On Error GoTo 0
Exit Function

GetSlope_Error:
    If bDebug Then Handle_Err Err, "GetSlope-GraphLite"
    Resume Next
End Function
