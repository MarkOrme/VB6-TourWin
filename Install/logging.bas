...
'Remeber that all the extra controls that is Start Button
'and the taskbar Icons are all situated in Taskbar
'which have the class name of Shell_TrayWnd
...
'-----------------------------------------------------------
' FUNCTION: fGetTaskBarDimensions
'
' Returns TRUE if successful
'-----------------------------------------------------------
'
Function fGetTaskBarDimensions(ByRef lHeigth As Long, ByRef lWidth As Long) As Boolean

Dim Wnd         As Long
Dim TaskbarDim  As RECT

Wnd = FindWindow("Shell_TrayWnd", vbNullString) 'Get the hWnd of the taskbar
GetWindowRect Wnd, TaskbarDim 'Fill the structure Rect
lWidth = TaskbarDim.Right - TaskbarDim.Left 'Width of the taskbar
lHeigth = TaskbarDim.Bottom - TaskbarDim.Top 'Height of the taskbar

fGetTaskBarDimensions = True
End Function
